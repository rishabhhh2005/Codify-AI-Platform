...
export default function ResultsScreen({ session, messages, elapsedSeconds, hintsUsed, onReset, submissionResults, questions }) {
  ...
  // Extract score from last AI message if present, or calculate from submission results
  const lastAiMsg = [...messages].reverse().find(m => m.role === 'assistant');
  
  let extractedScore = 0;
  if (submissionResults && questions && submissionResults.length > 0) {
    let totalPct = 0;
    submissionResults.forEach((res, idx) => {
      const q = questions[idx];
      const maxT = res?.total || q?.examples?.length || 1;
      const passed = res?.passed || 0;
      totalPct += (passed / maxT) * 100;
    });
    extractedScore = Math.round(totalPct / questions.length);
  } else {
    // Fallback exactly as it was
    const scoreMatch = lastAiMsg?.content?.match(/\b([0-9]{1,3})\/100\b|\bscore[:\s]+([0-9]{1,3})\b/i);
    extractedScore = scoreMatch ? parseInt(scoreMatch[1] || scoreMatch[2]) : 0;
  }
      ...
      <div className="w-full max-w-2xl space-y-8 relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        <div className="text-center space-y-4">
          ...
          <div className="space-y-1">
             ...
             <p className="text-neutral-500 text-sm font-medium tracking-wide">Analysis complete for {topic?.label} · {difficulty?.label}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <StatCard label="Accuracy" value="High" icon={Trophy} colorClass="text-emerald-400" />
        </div>
        ...
        <div className="bg-[#121214] border border-white/5 rounded-3xl p-6 shadow-2xl relative group">
          ...
          <div className="text-sm text-neutral-300 leading-relaxed font-sans line-clamp-4">
             {lastAiMsg ? lastAiMsg.content.replace(/```[\s\S]*?```/g, '').trim() : "Score based on automated remote execution with JDoodle against standard test cases. Manual AI evaluation was skipped or not requested."}
          </div>
